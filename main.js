(function(){
  ...
  function aggregatePlayers(){
    ...
    for(const key of Object.keys(players)){
      const p=players[key]||{};
      out[key]={ name:p.name||key, avatar:p.avatar||"", achievements:Array.isArray(p.achievements)?p.achievements:[], wins:0, losses:0, saldo:0, series:0, titles:0 };
    }
    ...
    for(const t of tours){
      ...
      for(const m of matches){
        ...
        if(!out[a]) out[a]={name:a,avatar:"",achievements:[],wins:0,losses:0,saldo:0,series:0,titles:0};
        if(!out[b]) out[b]={name:b,avatar:"",achievements:[],wins:0,losses:0,saldo:0,series:0,titles:0};
      ...
      if(Array.isArray(t.ranking) && t.ranking[0] && t.ranking[0].name){
        ...
        if(!out[champ]) out[champ]={name:champ,avatar:"",achievements:[],wins:0,losses:0,saldo:0,series:0,titles:0};
        out[champ].titles++;
      }
    }
    return out;
  ...
  function buildPlayerCard(p){
    ...
    const sub=el("p",{className:"player-sub"},[
      `${p.titles||0} título(s) · ${p.wins||0}V-${p.losses||0}D · saldo ${p.saldo>0?"+":""}${p.saldo||0}`
    ]);
